...
sub call {
  ...
  return $post_param ? $self->_post_req( $path, $post_param ) : $self->_get_req( $path );
}

sub _get_req {
  my ( $self, $path ) = @_;
  my $req = HTTP::Request->new( 'GET' => sprintf( '%s/%s.json', $self->base_uri, $path), HTTP::Headers->new( Content_Type => 'application/json', 'Accept' => 'application/json', ) );
...
sub _post_req {
  ...
  my $req = HTTP::Request->new(POST => sprintf( '%s/%s.json', $self->base_uri, $path) , HTTP::Headers->new( Content_Type => 'application/json', 'Accept' => 'application/json', ), );
  $req->authorization_basic($self->username, $self->password);
